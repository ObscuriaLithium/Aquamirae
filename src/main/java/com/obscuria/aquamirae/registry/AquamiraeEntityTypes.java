...
import com.obscuria.aquamirae.common.entity.projectile.*;
...
public interface AquamiraeEntityTypes {
	...
	RegistrySupplier<EntityType<ShadowMerchant>> SHADOW_MERCHANT = simple("shadow_merchant", ShadowMerchant::new, MobCategory.CREATURE,
			builder -> builder.clientTrackingRange(10).sized(0.6F, 1.95F));
	RegistrySupplier<EntityType<SpineArrow>> SPINE_ARROW = simple("spine_arrow", SpineArrow::new, MobCategory.MISC,
			builder -> builder.sized(0.5f, 0.5f).clientTrackingRange(4).updateInterval(20));
	RegistrySupplier<EntityType<AbyssalArrow>> ABYSSAL_ARROW = simple("abyssal_arrow", AbyssalArrow::new, MobCategory.MISC,
			builder -> builder.sized(0.5f, 0.5f).clientTrackingRange(4).updateInterval(20));
	RegistrySupplier<EntityType<IceShard>> ICE_SHARD = simple("ice_shard", IceShard::new, MobCategory.MISC,
			builder -> builder.sized(0.4f, 0.4f).clientTrackingRange(4).updateInterval(10));
	RegistrySupplier<EntityType<AbyssalShard>> ABYSSAL_SHARD = simple("abyssal_shard", AbyssalShard::new, MobCategory.MISC,
			builder -> builder.sized(0.4f, 0.4f).clientTrackingRange(4).updateInterval(10));
	...
	static void registerAttributes(EntityAttributeCreationEvent event) {
		...
		event.put(SHADOW_MERCHANT.get(), WanderingTrader.createMobAttributes().build());
	}
