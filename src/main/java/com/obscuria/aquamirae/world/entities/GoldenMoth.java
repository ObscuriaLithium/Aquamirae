...
public class GoldenMoth extends CreatureEntity implements IShipGraveyardEntity {
	...
	@Override public boolean hurt(DamageSource source, float amount) {
		...
		if (this.level instanceof ServerWorld) ((ServerWorld) this.level).sendParticles(AquamiraeParticleTypes.SHINE.get(),
	...
	@Override public void baseTick() {
		...
		if (this.getPersistentData().getDouble("shine") > 2) {
			...
			if (this.level instanceof ServerWorld)
				((ServerWorld) this.level).sendParticles(AquamiraeParticleTypes.SHINE.get(), this.getX(), this.getY(), this.getZ(),
						1, 0.1, 0.1, 0.1, 0.1);
		}
