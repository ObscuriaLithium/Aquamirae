	...
	public static class CaptainCorneliaItemLayer extends RenderLayer<CaptainCornelia, ModelCaptainCornelia> {
		...
		public void render(@NotNull PoseStack pose, @NotNull MultiBufferSource source, int i1, CaptainCornelia entity, float f1, float f2, float f3, float f4, float f5, float f6) {
			...
			if (!right.isEmpty()) {
				...
				Minecraft.getInstance().gameRenderer.itemInHandRenderer.renderItem(entity, right, ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, false, pose, source, i1);
				pose.popPose();
			}
			...
			if (!left.isEmpty()) {
				...
				pose.mulPose(Vector3f.XP.rotationDegrees(45.0F));
				pose.translate(0.0D, -0.15D, -0.65D);
				Minecraft.getInstance().gameRenderer.itemInHandRenderer.renderItem(entity, left, ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND, false, pose, source, i1);
				pose.popPose();
			}
		}
	}
