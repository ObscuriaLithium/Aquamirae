...
public class GoldenMothRenderer extends MobRenderer<GoldenMoth, ModelMoth<GoldenMoth>> {
	...
	public GoldenMothRenderer(EntityRendererProvider.Context context) {
		...
		this.addLayer(new MothWingsLayer<>(this, Aquamirae.key("textures/entity/golden_moth_wings.png")));
	}

	@Override
	public void render(GoldenMoth entity, float f1, float f2, PoseStack pose, MultiBufferSource source, int light) {
		super.render(entity, f1, f2, pose, source, light);
		Trail3DRenderer.of(entity.trail).render(entity, pose, source, Minecraft.getInstance().getPartialTick());
	}
