...
import com.obscuria.core.common.item.ability.AbilityHelper;
...
public class StormChakram extends ThrowableProjectile {
    ...
    public static StormChakram create(Level level, LivingEntity owner, ItemStack stack) {
        final var result = new StormChakram(AquamiraeEntityTypes.STORM_CHAKRAM.get(), level);
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        ...
        if (!level().isClientSide) {
            if (!player.getInventory().add(stack))
                WorldUtil.drop(level(), position(), stack);
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            updateSpinFactor();
            if (tickCount % 10 == 0) level().addParticle(AquamiraeParticleTypes.GHOST_SHINE.get(),
